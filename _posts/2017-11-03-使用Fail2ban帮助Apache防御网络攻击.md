...
mod\_security 相比 mod\_evasive 要更强大一些，作为一个 Web 应用防火墙（WAF），它使用规则引擎识别多种网络攻击行为模式，例如：SQL 注入，跨站攻击等，并可以改变 Request 和 Response 中的内容作出防御，它也工作在**应用级别**。安装和启用 mod\_security 的方式如下：
